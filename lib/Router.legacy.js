...
class Router {
		...
		for(let [key, route] of this.routes) {
			...
			try {
				...
				if(test) {
					...
					switch(r_type) {
						case 'endpt':
							global.emit('endpoint',this,r_key,route);
							rtn = await Router.handleRoute(route,method,req,res,options);
							break;
						case 'mdlwr':
							global.emit('middleware',this,r_key,route);
							rtn = await Router.handleRoute(route,method,req,res,options);
							break;
						case 'mount':
							global.emit('mount',this,r_key,route);
	 ...
	 *     name: [String]
	 *     options: [Object] options to give path-to-regex
	...
	addRoute(data, ...middleware) {
		...
		let key = `${type}:${typeof data.name === 'string' ? data.name : data.path}`;
